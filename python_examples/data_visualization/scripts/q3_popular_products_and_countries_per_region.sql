...
-- Used to gather data on the total sales of in a country within a region
, sales_by_country_and_region as (
	SELECT
		region
		, country_state
		, SUM(quantity) as country_and_region_total_quantity
...
-- A single query which will show the total sales for regions, product lines in regions, and countries within regions
-- as well as their respective rankings
-- The country with the highest sales will appear first (repeated)
-- The country within a region will also appear first (repeated)
-- Finally, the order on the far right will display the highest ranking product lines within the given region
SELECT
